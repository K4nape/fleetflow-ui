...
export default function Contracts() {
  ...
  const contracts: Contract[] = [
    ...
    },
    {
      ...
      notes: "Vėluoja grąžinti automobilį",
    },
    {
      id: "6",
      ...
      notes: "Klientas atšaukė rezervaciją",
    },
  ];
  ...
  const stats = {
    ...
    thisMonth: contracts.filter(c => new Date(c.createdAt).getMonth() === new Date().getMonth()).length,
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 pb-20 lg:pb-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Card className="p-3 sm:p-4 bg-gradient-to-br from-info/10 to-info/5 border-info/20">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-info/20">
              <Calendar className="h-4 w-4 sm:h-5 sm:w-5 text-info" />
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Šį mėnesį</p>
              <p className="text-lg sm:text-xl font-bold">{stats.thisMonth}</p>
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
          ...
          <Select>
            ...
            <SelectContent>
              ...
              <SelectItem value="completed">Užbaigtos</SelectItem>
              <SelectItem value="overdue">Vėluojančios</SelectItem>
              <SelectItem value="cancelled">Atšauktos</SelectItem>
            </SelectContent>
          </Select>
          <Select>
            <SelectTrigger className="bg-background/50 border-border/50 rounded-xl text-sm">
            ...
            >
              <div className="p-3">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className={`flex-1 flex items-center gap-2 px-2 py-1.5 rounded-lg ${paymentConfig.bg}`}>
                    <Euro className={`h-4 w-4 ${paymentConfig.color}`} />
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-medium ${paymentConfig.color}`}>{formatCurrency(contract.paidAmount)}</span>
                        <span className="text-xs text-muted-foreground">/ {formatCurrency(contract.totalAmount)}</span>
                      </div>
                      <Progress value={paymentProgress} className="h-1 mt-1" />
                    </div>
                  </div>
                </div>
